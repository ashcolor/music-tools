...
export type SoundType = "electronic" | "classic";

const SAMPLE_URLS: Partial<Record<SoundType, { normal: string; accent: string }>> = {
  classic: { normal: "/sounds/metronome.mp3", accent: "/sounds/metronome_accent.mp3" },
};
...
const SOUND_TYPES: SoundType[] = ["electronic", "classic"];
...
export function MetronomeProvider({ children }: { children: ReactNode }) {
  ...
  const sampleBuffersRef = useRef<
    Partial<Record<SoundType, { normal: AudioBuffer; accent: AudioBuffer }>>
  >({});
  const sampleLoadingRef = useRef<Partial<Record<SoundType, Promise<void>>>>({});

  const loadSampleBuffers = useCallback(async (type: SoundType) => {
    const urls = SAMPLE_URLS[type];
    if (!urls) return;
    if (sampleBuffersRef.current[type]) return;
    if (sampleLoadingRef.current[type]) {
      await sampleLoadingRef.current[type];
      return;
    }
    const ctx = audioContextRef.current;
    if (!ctx) return;
    const promise = (async () => {
      const [normalRes, accentRes] = await Promise.all([fetch(urls.normal), fetch(urls.accent)]);
      ...
      const [normal, accent] = await Promise.all([
        ctx.decodeAudioData(normalBuf),
        ctx.decodeAudioData(accentBuf),
      ]);
      sampleBuffersRef.current[type] = { normal, accent };
    })();
    sampleLoadingRef.current[type] = promise;
    try {
      await promise;
    } finally {
      delete sampleLoadingRef.current[type];
    }
  }, []);
  ...
  const playClick = useCallback((time: number, isAccent = false) => {
    ...
    if (SAMPLE_URLS[soundType]) {
      const buffers = sampleBuffersRef.current[soundType];
      if (!buffers) return;
  ...
  const start = useCallback(async () => {
    ...
    if (SAMPLE_URLS[stateRef.current.soundType]) {
      await loadSampleBuffers(stateRef.current.soundType);
    }
  ...
  const setSoundType = useCallback(
    (v: SoundType) => {
      syncDispatch({ type: "SET_SOUND_TYPE", value: v });
      if (SAMPLE_URLS[v] && audioContextRef.current) {
        void loadSampleBuffers(v);
      }
    },
    [syncDispatch, loadSampleBuffers],
  );
